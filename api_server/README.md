...
Для работы с API вам придётся добавить в таблицу `api_users` базы данных, имя пользователя и хеш пароля. Методов API для регистрации пользователей не предусмотрено. Вам придётся создать имя пользователя и пароль, а затем сгенерировать bcrypt хеш пароля с помощью flask-jwt-extended с учётом значения `JWT_SECRET_KEY` используемого сервером API. Затем вам необходимо сохранить выбранное имя пользователя и сгенерированный хеш пароля в таблицу `api_users` базы данных.
